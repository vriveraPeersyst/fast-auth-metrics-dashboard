...
`run-all.ts` runs **five collectors concurrently** via `Promise.all` (they hit disjoint upstreams and write to disjoint tables):
...
The three health collectors share `health-classifier.ts` for receipt-walking + RPC plumbing. Each collector does two bounded passes per cycle: **discover** (≤50 new rows from the last `DISCOVERY_LOOKBACK_DAYS` (1) that aren't yet classified, calls NEAR `tx` RPC, persists newest-first) and **retry** (≤25 `rpc_pending` rows older than 5 min with `retry_count < 10`, re-classifies). Sized so worst-case cycle time stays under ~150s even with full RPC timeout cascades; in steady state most calls succeed in ~200ms.
